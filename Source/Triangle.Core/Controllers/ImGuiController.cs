...
public unsafe class ImGuiController : Disposable
{
    ...
    /// </summary>
    public ImGuiController(TrContext context, IView view, IInputContext input) : this(context, view, input, null, null)
    {
    }
    ...
    /// </summary>
    public ImGuiController(TrContext context, IView view, IInputContext input, ImGuiFontConfig imGuiFontConfig) : this(context, view, input, imGuiFontConfig, null)
    {
    }
    ...
    /// </summary>
    public ImGuiController(TrContext context, IView view, IInputContext input, Action onConfigureIO) : this(context, view, input, null, onConfigureIO)
    {
    }
    ...
    /// </summary>
    public ImGuiController(TrContext context, IView view, IInputContext input, ImGuiFontConfig? imGuiFontConfig = null, Action? onConfigureIO = null)
    {
        _trContext = context;
        _gl = context.GL;
        _view = view;
    ...
    private void UpdateImGuiInput()
    {
        ...
        ScrollWheel wheel = _input.Mice[0].ScrollWheels[0];
        io.MouseWheel = wheel.Y;
        io.MouseWheelH = wheel.X;

        foreach (var key in _keyEnumArr)
        {
            if (key == Key.Unknown)
            {
                continue;
    ...
    private void CreateDeviceResources()
    {
        ...
        using TrShader vs = new(_trContext, TrShaderType.Vertex, vertexSource, false);
        using TrShader fs = new(_trContext, TrShaderType.Fragment, fragmentSource, false);

        _pipeline = new TrRenderPipeline(_trContext, [vs, fs]);
