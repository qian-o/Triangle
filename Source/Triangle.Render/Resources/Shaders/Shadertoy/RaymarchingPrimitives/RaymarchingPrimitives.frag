...
float sdRhombus(vec3 p, float la, float lb, float h, float ra)
{
    ...
    vec2 q =
        vec2(length(p.xz - 0.5 * b * vec2(1.0 - f, 1.0 + f)) * sign(p.x * b.y + p.z * b.x - b.x * b.y) - ra, p.y - h);
...
vec2 map(vec3 pos)
{
    ...
    {
        res = opU(res, vec2(sdCappedTorus((pos - vec3(0.0, 0.30, 1.0)) * vec3(1.0, -1.0, 1.0), vec2(0.866025, -0.5),
                                          0.25, 0.05),
                            25.0));
    ...
    if (sdBox(pos - vec3(1.0, 0.3, -1.0), vec3(0.35, 0.3, 2.5)) < res.x)
    {
        ...
        res = opU(res,
                  vec2(sdCapsule(pos - vec3(1.0, 0.00, -1.0), vec3(-0.1, 0.1, -0.1), vec3(0.2, 0.4, 0.2), 0.1), 31.9));
    ...
    if (sdBox(pos - vec3(-1.0, 0.35, -1.0), vec3(0.35, 0.35, 2.5)) < res.x)
    {
        ...
        res = opU(
            res,
            vec2(sdHorseshoe(pos - vec3(-1.0, 0.25, 1.0), vec2(cos(1.3), sin(1.3)), 0.2, 0.3, vec2(0.03, 0.08)), 11.5));
    }
    ...
    if (sdBox(pos - vec3(2.0, 0.3, -1.0), vec3(0.35, 0.3, 2.5)) < res.x)
    {
        ...
        res = opU(res, vec2(sdCylinder(pos - vec3(2.0, 0.14, -2.0), vec3(0.1, -0.1, 0.0), vec3(-0.2, 0.35, 0.1), 0.08),
                            31.2));
        res = opU(
            res, vec2(sdCappedCone(pos - vec3(2.0, 0.09, -1.0), vec3(0.1, 0.0, 0.0), vec3(-0.2, 0.40, 0.1), 0.15, 0.05),
                      46.1));
        res = opU(res,
                  vec2(sdRoundCone(pos - vec3(2.0, 0.15, 0.0), vec3(0.1, 0.0, 0.0), vec3(-0.1, 0.35, 0.1), 0.15, 0.05),
                       51.7));
...
void main()
{
    ...
    if (Uni_Vectors.Mouse.z == 1.0)
    {
        mo = Uni_Vectors.Mouse.xy / Uni_Vectors.Resolution.xy;
    }

    float time = 32.0 + Uni_Constants.Time * 1.5;
    ...
    for (int m = 0; m < ANTI_ALIASING; m++)
    {
        for (int n = 0; n < ANTI_ALIASING; n++)
        {
            ...
            vec2 px = (2.0 * (gl_FragCoord.xy + vec2(1.0, 0.0)) - Uni_Vectors.Resolution.xy) / Uni_Vectors.Resolution.y;
            vec2 py = (2.0 * (gl_FragCoord.xy + vec2(0.0, 1.0)) - Uni_Vectors.Resolution.xy) / Uni_Vectors.Resolution.y;
